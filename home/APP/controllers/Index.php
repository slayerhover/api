...
class IndexController extends CoreController {		
	...
	public function init() {	
		parent::init();		
		$this->url		=	$this->config['application']['scheme'].'://'.$this->config['application']['domain'];
		$this->pageurl  =	$this->config['application']['scheme'].'://'.($this->config['application']['domain']=="cfb.scsj.net.cn" ? "cfb.scsj.net.cn" : "h5.scsj.net.cn/cfb");
	}
	/**
	 *接口名称	APP欢迎页
	...
	public function syncTokenAction(){
		$now	=	time();
		foreach(DB::table('t_user')->where('token','<>','')->select('id','phone','type','code','inviter_id','inviter_code','saleuser_state','token')->get() as $v){
	...
	public function loginAction(){
		do{			
			...
			$user = DB::table('t_user')->where('phone','=',$phone)									   
									   ...
									   ->select('id','token','type','code','phone','salt','inviter_id','inviter_code','lockuntil','user_login_time','user_login_ip','saleuser_state')
									   ->first();	
			if(empty($user)){
	...
	***签名EOF***/
	...
	public function bindAction(){
		...
		$inputs	= array(			
				['name'=>'token',  'value'=>$token,	 'role'=>'required', 'msg'=>'登陆标识为空.'],
				['name'=>'inviter_code',  'value'=>$new_inviter_code,	 'role'=>'required', 'msg'=>'邀请码为空.'],
		);
		$result		= Validate::check($inputs);			
		if(!empty($result)){
			ret(1, $result, '输入参数有误.');
		...
		if($cuser['inviter_code']==''&&$cuser['inviter_id']==0){		
			...
			ret(0, ['inviter_code'=>$new_inviter_code], '绑定服务专员成功.');
		}
		ret(1, ['inviter_code'=>$new_inviter_code], '绑定失败.');
	}
